...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        const fetchCartItems = async () => {
            try {
                const response = await axios.get(`https://food-delivery-website-sepia-tau.vercel.app/api/cart/${user}`);
    ...
    const handleRemoveItem = async (itemId) => {
        try {
            const response = await axios.post(`https://food-delivery-website-sepia-tau.vercel.app/api/cart/removeItem`, { user, itemId });
                        ...
                        <ul>
                            {cartItems.map((item) => (
                                <li key={item._id} className="flex gap-4 items-center p-4 border-b">
                                    <img
                                        src={`https://food-delivery-website-sepia-tau.vercel.app/${item.image}`}
                                        alt={item.itemName}
